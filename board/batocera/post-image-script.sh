...
case "${BATOCERA_TARGET}" in
	...
	cp board/batocera/odroidn2/boot/logo.bmp.gz ${BINARIES_DIR}/boot   || exit 1

        # boot.tar.xz
        echo "creating boot.tar.xz"
        (cd "${BINARIES_DIR}/boot" && tar -cJf "${BATOCERA_BINARIES_DIR}/boot.tar.xz" boot.ini boot batocera-boot.conf logo.bmp.gz) || exit 1
